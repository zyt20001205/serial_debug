...
class ViewWidget(QMenu):
    def __init__(self, show_list: list):
        ...
        from gui_module import port_log_dock_widget, port_status_dock_widget, single_send_dock_widget, advanced_send_dock_widget, file_send_dock_widget, \
            command_shortcut_dock_widget, data_collect_dock_widget
        super().__init__()
        # dock widget view checkbox
        self.port_log_checkbox = QAction("port log", self)
        self.port_log_checkbox.setCheckable(True)
        self.port_log_checkbox.triggered.connect(lambda checked: dock_update(port_log_dock_widget, checked))
        self.addAction(self.port_log_checkbox)
    ...
    def view_init(self, show_list: list) -> None:
        if "port_log" in show_list:
            self.port_log_checkbox.setChecked(True)
