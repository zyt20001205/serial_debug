...
class CommandShortcutWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self.shortcut_table = self.ShortcutTableWidget()
        self.shortcut_table.clear_signal.connect(self.shortcut_clear)
    ...
    class ShortcutTableWidget(QTableWidget):
        ...
        def startDrag(self, supportedActions):
            ...
            # write type
            stream.writeQString(self.cellWidget(self.source_index, 2).text())
            # write command
            stream.writeQString(self.cellWidget(self.source_index, 4).text())
            # create mime data
            mime_data = QMimeData()
        ...
        def row_relocation(self):
            ...
            if source_index < target_index:
                ...
                self.setCellWidget(target_index + 1, 1, send_button)
                type_lineedit = QLineEdit()
                type_lineedit.setText(self.cellWidget(source_index, 2).text())
                self.setCellWidget(target_index + 1, 2, type_lineedit)
                function_lineedit = QLineEdit()
                function_lineedit.setText(self.cellWidget(source_index, 3).text())
                self.setCellWidget(target_index + 1, 3, function_lineedit)
                command_lineedit = QLineEdit()
                command_lineedit.setText(self.cellWidget(source_index, 4).text())
                self.setCellWidget(target_index + 1, 4, command_lineedit)
                suffix_lineedit = QLineEdit()
                suffix_lineedit.setText(self.cellWidget(source_index, 5).text())
                self.setCellWidget(target_index + 1, 5, suffix_lineedit)
                format_combobox = QLineEdit()
                format_combobox.setText(self.cellWidget(source_index, 6).text())
                self.setCellWidget(target_index + 1, 6, format_combobox)
                # remove source row
                ...
                send_button.setIcon(QIcon("icon:send.svg"))
                send_button.clicked.connect(shared.command_shortcut_widget.shortcut_send)
                self.setCellWidget(target_index, 1, send_button)
                type_lineedit = QLineEdit()
                type_lineedit.setText(self.cellWidget(source_index + 1, 2).text())
                self.setCellWidget(target_index, 2, type_lineedit)
                function_lineedit = QLineEdit()
                function_lineedit.setText(self.cellWidget(source_index + 1, 3).text())
                self.setCellWidget(target_index, 3, function_lineedit)
                command_lineedit = QLineEdit()
                command_lineedit.setText(self.cellWidget(source_index + 1, 4).text())
                self.setCellWidget(target_index, 4, command_lineedit)
                suffix_lineedit = QLineEdit()
                suffix_lineedit.setText(self.cellWidget(source_index + 1, 5).text())
                self.setCellWidget(target_index, 5, suffix_lineedit)
                format_combobox = QLineEdit()
                format_combobox.setText(self.cellWidget(source_index + 1, 6).text())
                self.setCellWidget(target_index, 6, format_combobox)
                # remove source row
    ...
    def command_shortcut_gui(self):
        ...
        self.shortcut_table.setColumnWidth(1, 30)
        self.shortcut_table.setColumnWidth(2, 80)
        self.shortcut_table.setColumnWidth(3, 80)
        header.setSectionResizeMode(4, QHeaderView.ResizeMode.Stretch)
        self.shortcut_table.setColumnWidth(5, 40)
        self.shortcut_table.setColumnWidth(6, 40)
        ...
        for i in range(shared.shortcut_count):
            ...
            self.shortcut_table.setCellWidget(i, 1, send_button)
            # type lineedit
            type_lineedit = QLineEdit()
            type_lineedit.setText(shared.shortcut[i]["type"])
            type_lineedit.setStyleSheet(f"background-color: {color};")
            self.shortcut_table.setCellWidget(i, 2, type_lineedit)
            # function lineedit
            function_lineedit = QLineEdit()
            function_lineedit.setText(shared.shortcut[i]["function"])
            function_lineedit.setStyleSheet(f"background-color: {color};")
            self.shortcut_table.setCellWidget(i, 3, function_lineedit)
            # command lineedit
            command_lineedit = QLineEdit()
            command_lineedit.setText(shared.shortcut[i]["command"])
            command_lineedit.setStyleSheet(f"background-color: {color};")
            self.shortcut_table.setCellWidget(i, 4, command_lineedit)
            # suffix lineedit
            suffix_lineedit = QLineEdit()
            suffix_lineedit.setText(shared.shortcut[i]["suffix"])
            suffix_lineedit.setStyleSheet(f"background-color: {color};")
            self.shortcut_table.setCellWidget(i, 5, suffix_lineedit)
            # format combobox
            format_combobox = QLineEdit()
            format_combobox.setText(shared.shortcut[i]["format"])
            format_combobox.setStyleSheet(f"background-color: {color};")
            self.shortcut_table.setCellWidget(i, 6, format_combobox)
    ...
    def shortcut_send(self):
        ...
        for row in range(self.shortcut_table.rowCount()):
            if self.shortcut_table.cellWidget(row, 1) == self.sender():
                index = row
                break
        command = self.shortcut_table.cellWidget(index, 4).text()
        suffix = self.shortcut_table.cellWidget(index, 5).text()
        format = self.shortcut_table.cellWidget(index, 6).text()
        if self.shortcut_table.cellWidget(index, 2).text() == "single":
            shared.single_send_widget.single_send(command, suffix, format)
        else:
            buffer = eval(command)
            thread_id = self.shortcut_table.cellWidget(index, 3).text()
    ...
    def shortcut_save(self, index, type, command, suffix, format):
        index -= 1
        self.shortcut_table.cellWidget(index, 2).setText(type)
        self.shortcut_table.cellWidget(index, 4).setText(command)
        self.shortcut_table.cellWidget(index, 5).setText(suffix)
        self.shortcut_table.cellWidget(index, 6).setText(format)
    ...
    def shortcut_paint(self):
        ...
        if row == -1:
            ...
            if color.isValid():
                color = color.name()
                shared.shortcut[row]["color"] = color
    ...
    def shortcut_clear(self):
        ...
        if row == -1:
            ...
            self.shortcut_table.cellWidget(row, 6).setText("")
            # clear color
            shared.shortcut[row]["color"] = "white"
            for col in range(1, self.shortcut_table.columnCount()):
                self.shortcut_table.cellWidget(row, col).setStyleSheet("background-color: white;")

    def shortcut_config_save(self):
        for i in range(shared.shortcut_count):
            shared.shortcut[i]["type"] = self.shortcut_table.cellWidget(i, 2).text()
            shared.shortcut[i]["function"] = self.shortcut_table.cellWidget(i, 3).text()
            shared.shortcut[i]["command"] = self.shortcut_table.cellWidget(i, 4).text()
            shared.shortcut[i]["suffix"] = self.shortcut_table.cellWidget(i, 5).text()
            shared.shortcut[i]["format"] = self.shortcut_table.cellWidget(i, 6).text()